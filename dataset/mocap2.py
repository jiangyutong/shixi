...
class Mocap(BaseDataset):
    ...
    def _process_points(self, data):
        ...
        p3d_world_orig = np.array([j['trans'] for jid, j in enumerate(data['joints'])])
        camera=data['camera']
        # ------------------- Filter joints -------------------

        p2d = np.empty([len(config.skel), 2], dtype=p2d_orig.dtype)
        p3d = np.empty([len(config.skel), 3], dtype=p2d_orig.dtype)
        p3d_world = np.empty([len(config.skel), 3], dtype=p2d_orig.dtype)

        for jid, j in enumerate(config.skel.keys()):
            p2d[jid] = p2d_orig[joint_names[j]]
            p3d[jid] = p3d_orig[joint_names[j]]
            p3d_world[jid] = p3d_world_orig[joint_names[j]]

        p3d /= self.CM_TO_M
        p3d_world /= self.CM_TO_M

        return camera,p2d, p3d , p3d_world
    ...
    def __getitem__(self, index):
        ...
        camera,p2d, p3d, p3d_w = self._process_points(data)

        # get action name
        action = data['action']

        # get subject
        subject = json_path.split("/")[-5]
        env = json_path.split("/")[-4]
        if self.transform:
            img = self.transform({'image': img})['image']
            p3d = self.transform({'joints3D': p3d})['joints3D']
            p2d = self.transform({'joints2D': p2d})['joints2D']

        return img, p2d, p3d, p3d_w, action,subject,env,img_path,camera
